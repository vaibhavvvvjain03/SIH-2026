...
window.DataContract.fetchAttribution = async function(address, chain) {
  ...
  const ranked_vasps = shuffled
    .map((name) => ({ name, score: +(0.4 + rand() * 0.55).toFixed(3) }))
  ...
  return {
    ...
    chain,
    ranked_vasps,
    ml_score: +(0.5 + rand() * 0.45).toFixed(2),
    graph_score: +(0.5 + rand() * 0.45).toFixed(2),
    transaction_score: +(0.5 + rand() * 0.45).toFixed(2),

...
function renderResults(prediction,transactions,address){resultsEl.hidden=false;metricFused.textContent=formatScore(prediction.fused_score);metricMl.textContent=formatScore(prediction.ml_score);metricGraph.textContent=formatScore(prediction.graph_score);metricTx.textContent=formatScore(prediction.transaction_score);
 ...
 if(prediction.unknown_or_insufficient_evidence){candidateCount.textContent="NO ASSERTION";candidatesList.innerHTML=`<div class="unknown-state"><strong>INSUFFICIENT EVIDENCE FOR VASP ATTRIBUTION</strong><p>The wallet did not match known VASP infrastructure with sufficient confidence. The console refuses to force a candidate.</p></div>`}
 else{candidateCount.textContent=`${prediction.ranked_vasps.length} CANDIDATE${prediction.ranked_vasps.length===1?"":"S"}`;candidatesList.innerHTML="";const maxScore=Math.max(...prediction.ranked_vasps.map(v=>v.score),.0001);prediction.ranked_vasps.forEach((vasp,index)=>{const card=document.createElement("div");card.className = "candidate" + (vasp.graph_support ? " graph-backed" : "");card.innerHTML=`<div class="candidate-top"><span class="candidate-name"><span class="candidate-rank">0${index+1}</span>${escapeHtml(vasp.name)}</span><div class="candidate-meta">${vasp.graph_support ? '<span class="graph-flag" title="Supported by transaction graph evidence">◈ GRAPH</span>' : ''}<span class="candidate-score">${vasp.score.toFixed(3)}</span><span class="candidate-arrow" aria-hidden="true">▾</span></div></div><div class="candidate-bar-track"><div class="candidate-bar-fill" style="width:${(vasp.score/maxScore)*100}%"></div></div><div class="candidate-why"><div class="candidate-why-inner"><div class="candidate-score-breakdown"><span>ML · ${formatScore(prediction.ml_score)}</span><span>GRAPH · ${formatScore(prediction.graph_score)}</span><span>TX · ${formatScore(prediction.transaction_score)}</span></div><ul>${prediction.evidence.map(e=>`<li>${escapeHtml(e)}</li>`).join("")}</ul></div></div>`;card.addEventListener("click",()=>card.classList.toggle("expanded"));candidatesList.appendChild(card)})}
 const vaspNames=prediction.ranked_vasps?prediction.ranked_vasps.map(v=>v.name):[];renderTransactionGraph(graphContainer,{rootAddress:address,transactions,vaspNames});renderTimeline(timelineContainer,{rootAddress:address,transactions});
}
...
function tinyPause(){return new Promise(r=>setTimeout(r,260))}function setStatus(msg){statusMessage.textContent=msg}function escapeHtml(str){const div=document.createElement("div");div.textContent=str;return div.innerHTML}
